# pages/9_QMS_&_Audit_Readiness.py
...
with st.expander("🌐 Manager's View: Upholding Quality and Ensuring Compliance", expanded=True):
    st.markdown("""
    As the Manager of Validation, my commitment to the Quality Management System (QMS) is non-negotiable. This dashboard is my primary interface for overseeing formal quality events that involve my team, ensuring we meet our compliance obligations and are perpetually "audit-ready." It allows me to manage investigations, monitor CAPA effectiveness, and confidently represent my department's state of control to both internal and external auditors.

    **Key Responsibilities & Regulatory Imperatives:**
    - **CAPA (21 CFR 820.100 & ISO 13485, 8.5.2):** This dashboard provides a consolidated view of all Corrective and Preventive Actions where the Validation team is a key stakeholder. I use this to manage timelines, ensure robust root cause analysis, and verify that corrective actions are implemented and effective.
    - **Nonconformance (21 CFR 820.90):** The "Active Investigations" tracker is critical for managing my team's documented response to validation study anomalies or deviations before a formal CAPA is initiated.
    - **Audit Support:** This page is designed to be an "auditor's entry point." It provides a clear, real-time status of any validation-related quality record, demonstrating a controlled process for handling non-conformances and driving continuous improvement.
    """)
...
col1.metric("Open Validation-Related CAPAs", f"{total_open_capas}")
...
with tab1:
    ...
    st.caption("Documented assessments of anomalies from validation studies, deviations, or post-launch monitoring.")
    if not inv_df.empty:
        ...
        st.success("No active validation-led investigations are currently open.")

with tab2:
    st.header("Formal Validation Corrective & Preventive Action (CAPA) Management")
    ...
    with st.expander("🔬 Manager's Analysis & Preventive Action"):
        st.markdown("""
        ...
        - **Source Analysis:** Analyzing the origin of quality events is a primary input for **Preventive Action**. A high count from "Post-Launch Complaint Trend" may indicate a gap in our pre-market validation. A high count from "Internal Audit Finding" points directly to a need for internal process improvement and retraining.
