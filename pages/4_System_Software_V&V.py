...
from utils import generate_traceability_matrix_data
...
# --- Project Selection ---
# In a real app, this would be dynamic. For this demo, we use a static example.
...
rtm_df = generate_traceability_matrix_data() # Using a generic RTM for this view
sw_rtm_df = rtm_df[rtm_df['Requirement Type'].str.contains("Software|Risk-Ctrl")].copy()

# KPI for Software RTM
total_reqs = len(sw_rtm_df)
pass_count = (sw_rtm_df['Test Result'] == 'Pass').sum()
...
st.dataframe(sw_rtm_df.style.applymap(style_rtm_status, subset=['Test Result']), use_container_width=True, hide_index=True)
...
st.caption("Tracking the lifecycle of defects found during V&V testing.")

# Mock data for defect tracking
defect_data = {
    'Defect ID': ['DEF-055', 'DEF-056'],
    'Associated Test Case': ['TC-FLAG-01-03', 'TC-REG-088'],
    'Description': [
        'Software does not flag results with a Ct > 37 under specific edge conditions involving manual entry.',
        'Legacy report format shows incorrect timestamp after patch is applied (regression bug).'
    ],
...
with st.expander("Director's Role in Defect Management"):
    st.markdown("""
    While I do not manage individual defects, I oversee the defect management process to ensure its effectiveness. My key concerns are:
    1.  **Defect Triage Process:** Is there a formal process to assess the severity of each defect and prioritize its resolution based on risk?
    2.  **Turnaround Time:** Are defects being resolved in a timely manner to not impede project timelines? A growing backlog of open, high-severity defects is a major red flag.
    3.  **V&V Retest:** Does our process ensure that once a defect is "fixed" by development, it is formally re-tested by the V&V team using the original failing test case? A fix is not complete until V&V confirms it.
    """)
