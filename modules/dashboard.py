# modules/dashboard.py (Final Production Version)
...
def render_page():
    ...
    with tab2:
        ...
        project_to_forecast = st.selectbox("Select a Project to Forecast", options=projects_df['Project'], help="Choose a project to see its forecasted completion date based on current velocity.")
                    ...
                    if model and forecast is not None:
                        ...
                        with f_col2:
                            if forecasted_date:
                                ...
                                st.metric(label="Forecasted Completion Date", value=forecasted_date.strftime('%Y-%m-%d'), delta=f"{delta_days} days from plan", delta_color="inverse" if delta_days <= 0 else "normal")
                            else:
                                st.metric(label="Forecasted Completion Date", value="> 90 days")
                        fig = plot_plotly(model, forecast)
                        fig.update_layout(title=f"Forecast for '{project_to_forecast}' Progress", xaxis_title="Date", yaxis_title="Completion (%)")
