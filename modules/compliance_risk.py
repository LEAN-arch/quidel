# modules/compliance_risk.py (Final Guaranteed Version)
...
@helpers.role_required('director')
def render_page():
    ...
    try:
        ...
        with tab1:
            ...
            if audit_trail_query:
                audit_data = [{"Timestamp (UTC)": log.timestamp.strftime('%Y-%m-%d %H:%M:%S'), "User": name, "Action": log.action, "Record Type": log.record_type, "Record ID": log.record_id, "Details": log.details} for log, name in audit_trail_query]
                st.dataframe(pd.DataFrame(audit_data), use_container_width=True, hide_index=True)
            else: st.info("No audit records found.")
        
        with tab2:
            st.subheader("Product Risk Management (FMEA)")
            st.info("This section would be built out with a dedicated 'Risk' table in the database.")
        ...
        with tab3:
            ...
            if st.button(f"🚀 Generate & Download Package for {submission_project_name}", type="primary"):
                with st.spinner("Bundling submission package... Please wait."):
                    ...
                    zip_buffer = helpers.create_submission_zip(project.name, pd.DataFrame(), protocols_df, pd.DataFrame())
                    user = helpers.get_current_user()
                    helpers.log_action(user.id, "Generated Regulatory Package", f"Project: {project.name}")
                    st.download_button(label="✅ Download Ready! Click Here.", data=zip_buffer, file_name=f"{project.name}_Regulatory_Package.zip", mime="application/zip")
    finally: db.close()
