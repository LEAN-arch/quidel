...
def render_page():
    """Renders the compliance and risk management hub."""
    ...
    tab1, tab2, tab3 = st.tabs([
        "🛡️ Product Risk Management (FMEA)",
        "🔎 Audit Trail & Change Control",
        "🗂️ Regulatory Package Builder"
    ])
    
    # --- TAB 1: PRODUCT RISK MANAGEMENT (FMEA) ---
    with tab1:
        ...
        # Display an editable FMEA table
    ...
    # --- TAB 2: AUDIT TRAIL & CHANGE CONTROL ---
    with tab2:
        ...
        eco_data = {
            'ECO Number': ['ECO-2024-034', 'ECO-2024-038'],
            'Change Description': ['Update consumable plastic material', 'Software patch for UI bug'],
            'Required V&V': ['Material Biocompatibility (re-verify)', 'Regression Testing Suite'],
            'V&V Status': ['In Progress', 'Completed']
        }
        eco_df = pd.DataFrame(eco_data)
        st.dataframe(eco_df, use_container_width=True, hide_index=True)

    # --- TAB 3: REGULATORY PACKAGE BUILDER ---
    with tab3:
        ...
        col1, col2 = st.columns(2)
        with col1:
            submission_project = st.selectbox(
                "Select Project for Submission", 
                options=st.session_state.projects_df['Project'].unique()
            )
        with col2:
