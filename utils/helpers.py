# utils/helpers.py (CORRECTED AND REFORMATTED)
...
def analyze_linearity(data_df):
    slope, intercept, r_value, _, _ = stats.linregress(data_df['Expected'], data_df['Observed'])
    results = {
        'N': len(data_df),
        'Slope': f"{slope:.4f}",
...
def generate_ppt_report(protocol_data, analysis_results, analysis_fig):
    ...
    txBox = slide.shapes.add_textbox(Inches(0.5), Inches(1.5), Inches(9.0), Inches(5.5))
    tf = txBox.text_frame
    tf.clear() 

    tf.paragraphs[0].text = f"Project: {protocol_data['Project']}"
    tf.add_paragraph().text = f"Acceptance Criteria: {protocol_data['Acceptance_Criteria']}"
    
    p_status = tf.add_paragraph()
    p_status.text = f"Status: {protocol_data['Status']}"
    p_status.space_before = Inches(0.2)
    
    p_results_header = tf.add_paragraph()
    p_results_header.text = "Execution Results:"
    p_results_header.space_before = Inches(0.5)

    # *** THIS IS THE CORRECTED SECTION ***
    # Replaced the invalid list comprehension with a standard for loop.
    ...
    slide.shapes.title.text = "Graphical Analysis"
...
def create_risk_bubble_chart(risk_df):
    ...
    fig = px.scatter(
        risk_df, x="Severity", y="Occurrence", size="RPN", color="Project",
        hover_name="Failure_Mode", size_max=60, title="Product Risk Landscape (FMEA)",
...
def create_failure_pareto_chart(protocols_df):
    ...
    fig.add_trace(go.Scatter(x=counts['Reason'], y=counts['Cumulative Pct'], name='Cumulative %', yaxis='y2', line=dict(color='red', width=2)))
    
    fig.update_layout(
        title="Root Causes of Test Failure (Pareto Analysis)",
        xaxis_title="Failure Reason", yaxis_title="Number of Occurrences",
        yaxis2=dict(title="Cumulative Percentage (%)", overlaying='y', side='right', range=[0, 105], showgrid=False),
        legend=dict(x=0.01, y=0.98), barmode='group'
    )
    return fig
