# utils/helpers.py (Final Complete Version)
...
def log_action(user_id: int, action: str, details: str = "", record_type: str = None, record_id: int = None):
    ...
    db = SessionLocal()
    new_log = AuditLog(user_id=user_id, action=action, details=details, record_type=record_type, record_id=record_id)
    db.add(new_log)
    db.commit()
    db.close()
...
def analyze_precision(data_df):
    """Performs precision analysis."""
    if 'Value' not in data_df.columns: return None, "Error: 'Value' column not found."
...
def create_enhanced_gantt(df):
    """Creates a Gantt chart with completion percentages."""
    if df.empty:
    ...
    gantt_df = df.copy()
    gantt_df.rename(columns={'name': 'Task','start_date': 'Start','finish_date': 'Finish','Pct_Complete': 'Completion_pct'}, inplace=True)
    gantt_df['Completion_pct'] = gantt_df['Completion_pct'] / 100
    fig = px.timeline(gantt_df,x_start="Start",x_end="Finish",y="Task",color="status",custom_data=['owner_id', 'Completion_pct'],title="Project Portfolio Status & Progress")
    fig.update_traces(text=gantt_df['Completion_pct'].apply(lambda x: f'{x:.0%}'), textposition='inside')
    fig.update_layout(xaxis_title="Timeline", yaxis_title="Project"); return fig

def create_risk_bubble_chart(risk_df):
    """Creates a risk bubble chart."""
    if risk_df.empty:
    ...
    fig=px.scatter(risk_df,x="Severity",y="Occurrence",size="RPN",color="Project",hover_name="Failure_Mode",size_max=60,title="Product Risk Landscape (FMEA)",labels={"Severity":"Severity (Impact)","Occurrence":"Likelihood of Occurrence"})
    fig.add_shape(type="rect",x0=6.5,y0=3.5,x1=10,y1=10,line=dict(color="Red",width=2,dash="dash"),fillcolor="rgba(255,0,0,0.1)")
    fig.add_annotation(x=9.5,y=9.5,text="High-Risk Zone",showarrow=False,xanchor='right',yanchor='top'); return fig
...
def create_submission_zip(project_name, project_reqs, project_protocols, project_risks):
    """Generates a zip archive for regulatory submission."""
...
def generate_prophet_history(project_data):
    ...
    if len(date_range) == 0: return pd.DataFrame(columns=['ds','y'])
    y_ideal=np.linspace(0,progress_to_date,len(date_range)); noise=np.random.normal(0,2,len(date_range))
    y_actual=np.clip(y_ideal+noise,0,100)
    history_df=pd.DataFrame({'ds':date_range,'y':y_actual}); return history_df
...
def run_prophet_forecast(history_df, future_days=90):
    ...
    history_df['cap']=100; m=Prophet(growth='logistic'); m.fit(history_df)
    future=m.make_future_dataframe(periods=future_days); future['cap']=100
    forecast=m.predict(future); return m,forecast
