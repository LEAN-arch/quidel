...
def plot_protocol_completion_burndown():
    days = np.arange(1, 31)
    planned = np.linspace(100, 0, 30)
    actual = planned + np.random.randn(30).cumsum() * 1.5
    actual = np.clip(actual, 0, 100)
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=days, y=planned, mode='lines', name='Planned Burndown', line=dict(dash='dash')))
    fig.add_trace(go.Scatter(x=days, y=actual, mode='lines+markers', name='Actual Burndown'))
    fig.update_layout(title='Protocol Completion Burndown Chart (Project ImmunoPro-A)', xaxis_title='Sprint Day', yaxis_title='% Protocols Remaining')
    return fig

def plot_pass_rate_heatmap():
    data = {'Test Type': ['LoD', 'Linearity', 'Specificity', 'Precision', 'Robustness'],
            'Pass Rate (%)': [95, 98, 92, 100, 88]}
    df = pd.DataFrame(data)
    fig = px.bar(df, x='Pass Rate (%)', y='Test Type', orientation='h', title='Pass Rate by Test Type', text='Pass Rate (%)')
    fig.update_traces(texttemplate='%{text}%', textposition='inside')
    return fig

def plot_retest_pareto():
    data = {'Reason': ['Operator Error', 'Reagent Lot Variability', 'Instrument Drift', 'Sample Prep Issue', 'Software Glitch'],
            'Count': [15, 9, 5, 3, 1]}
    df = pd.DataFrame(data).sort_values(by='Count', ascending=False)
    df['Cumulative Pct'] = (df['Count'].cumsum() / df['Count'].sum()) * 100
    fig = go.Figure()
    fig.add_trace(go.Bar(x=df['Reason'], y=df['Count'], name='Re-test Count'))
    fig.add_trace(go.Scatter(x=df['Reason'], y=df['Cumulative Pct'], name='Cumulative %', yaxis='y2'))
    fig.update_layout(title="Pareto Chart of Re-test Causes", yaxis2=dict(overlaying='y', side='right', title='Cumulative %'))
    return fig

def plot_trace_coverage_sankey():
    fig = go.Figure(go.Sankey(
        node=dict(pad=15, thickness=20, line=dict(color="black", width=0.5), label=["URS-01", "URS-02", "SRS-01", "DVP-01", "DVP-02", "UNCOVERED"]),
        link=dict(
            source=[0, 1, 2, 2],
            target=[3, 4, 3, 4],
            value=[8, 4, 2, 8],
            label=["Covers Sensitivity", "Covers Specificity", "Covers Speed", "Covers Speed"]
        )))
    fig.update_layout(title_text="Requirements Trace Coverage (Sankey Diagram)")
    return fig
...
def plot_rpn_waterfall():
    fig = go.Figure(go.Waterfall(
        ...
        x = ["Initial Risk (FP)", "Mitigation 1", "Subtotal", "Initial Risk (FN)", "Mitigation 2", "Final Risk Portfolio"],
        textposition = "outside",
        y = [120, -40, 0, 80, -60, 0],
        connector = {"line":{"color":"rgb(63, 63, 63)"}},
    ))
    fig.update_layout(title = "FMEA Risk Reduction (RPN Waterfall)", showlegend = True)
    return fig

# --- II. Equipment Validation ---
def plot_validation_gantt_baseline():
    df = pd.DataFrame([
        dict(Task="FAT", Start='2023-01-01', Finish='2023-01-10', Type="Planned"),
        dict(Task="FAT", Start='2023-01-01', Finish='2023-01-12', Type="Actual"),
        dict(Task="SAT", Start='2023-01-15', Finish='2023-01-20', Type="Planned"),
        dict(Task="SAT", Start='2023-01-18', Finish='2023-01-22', Type="Actual"),
        dict(Task="IQ", Start='2023-01-21', Finish='2023-01-25', Type="Planned"),
        dict(Task="IQ", Start='2023-01-23', Finish='2023-01-25', Type="Actual"),
    ])
    fig = px.timeline(df, x_start="Start", x_end="Finish", y="Task", color="Type", title="Validation On-Time Rate (Gantt vs Baseline)")
    return fig

def plot_sat_to_pq_violin():
    df = pd.DataFrame({
        'Days': np.concatenate([np.random.normal(20, 5, 20), np.random.normal(35, 8, 15)]),
        'Equipment Type': ['Analyzer'] * 20 + ['Sample Prep'] * 15
    })
    fig = px.violin(df, y="Days", x="Equipment Type", color="Equipment Type", box=True, points="all",
                    title="Time from SAT to PQ Approval (by Equipment Type)")
    return fig

# --- III. Team & Project ---
def plot_protocol_review_cycle_histogram():
    data = np.random.gamma(4, 2, 100)
    fig = px.histogram(data, title="Protocol Review Cycle Time (Draft to Approved)", labels={'value': 'Days'})
    return fig

def plot_training_donut():
    labels = ['ISO 13485','GAMP5','21 CFR 820', 'GDP', 'Not Started']
    values = [100, 95, 98, 90, 5]
    fig = go.Figure(data=[go.Pie(labels=labels, values=values, hole=.4, title="Training Completion Rate")])
    return fig
...
def plot_rft_gauge():
    fig = go.Figure(go.Indicator(
        mode = "gauge+number",
        value = 82,
        title = {'text': "Right-First-Time (RFT) Protocol Execution"},
        gauge = {'axis': {'range': [None, 100]}, 'bar': {'color': "cornflowerblue"}}))
    return fig

def plot_capa_funnel():
    fig = go.Figure(go.Funnel(
        y = ["Identified", "Investigation", "Root Cause Analysis", "Implementation", "Effectiveness Check"],
        x = [100, 80, 65, 60, 55],
        textinfo = "value+percent initial"))
    fig.update_layout(title="CAPA Closure Effectiveness Funnel")
    return fig

# --- V. Statistical Methods ---
def run_anova_ttest(add_shift):
    group_a = np.random.normal(10, 2, 30)
    group_b_mean = 10.5 if not add_shift else 12.5
    group_b = np.random.normal(group_b_mean, 2, 30)
    df = pd.DataFrame({'Group A': group_a, 'Group B': group_b})
    fig = px.box(df, title="Performance Comparison (Lot A vs Lot B)")
    t_stat, p_value = stats.ttest_ind(group_a, group_b)
    result = f"**T-test Result:** p-value = {p_value:.4f}. "
...
def run_regression_analysis():
    rpn = np.random.randint(20, 150, 50)
    failure_prob = rpn / 200
    failures = np.random.binomial(1, failure_prob)
    df = pd.DataFrame({'RPN': rpn, 'Failure Occurred': failures})
    df['Failure Occurred'] = df['Failure Occurred'].astype('category')
...
def run_descriptive_stats():
    ...
    df = pd.DataFrame(data, columns=["LoD Measurement (copies/mL)"])
    ...
    result = f"**Mean:** {mean:.2f} | **Std Dev:** {std:.2f} | **%CV:** {cv:.2f}%"
    return fig, result

def run_control_charts():
    # Create subgroups
    data = [np.random.normal(10, 0.5, 5) for _ in range(20)]
    data[15:] = [np.random.normal(10.8, 0.5, 5) for _ in range(5)] # Introduce a shift
    ...
    x_bar_ucl = x_bar_cl + 3 * (df['range'].mean() / 2.326) # A2 factor for n=5
    x_bar_lcl = x_bar_cl - 3 * (df['range'].mean() / 2.326)
    ...
    fig.update_layout(title="X-bar Control Chart for Process Monitoring")
    return fig

def run_kaplan_meier():
    # Simulate survival data
    time_to_failure = np.random.weibull(2, 50) * 24 # Months
    observed = np.random.binomial(1, 0.8, 50) # 80% have failed, 20% are censored
    df = pd.DataFrame({'Months': time_to_failure, 'Observed': observed})
    ...
    fig = px.line(df, x='Months', y='Survival', title="Kaplan-Meier Survival Plot for Shelf-Life Validation", markers=True)
    fig.update_yaxes(range=[0, 1.05])
    return fig, "**Conclusion:** The estimated median shelf-life (time to 50% survival) is approximately 21 months."

def run_monte_carlo():
    n_sims = 5000
    # Task durations are uncertain (triangular distribution)
    task1_times = np.random.triangular(left=8, mode=10, right=15, size=n_sims)
    task2_times = np.random.triangular(left=15, mode=20, right=30, size=n_sims)
    task3_times = np.random.triangular(left=5, mode=8, right=12, size=n_sims)
    total_times = task1_times + task2_times + task3_times
